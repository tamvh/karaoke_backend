...
ZErrorCode ZSessionService::validate(const std::string& token, int64_t& userId) const {
...
ZErrorCode ZSessionService::validate_jwt(const std::string& jwt, int64_t& userId) const {
...
/// @return ZErrorCode::Authentication_*
ZErrorCode ZSessionService::create(int64_t userId, std::string& token) {
...
ZErrorCode ZSessionService::create_jwt(int64_t userId, const std::string& claims, std::string& token) {
//    SessionData session_creator(aesPassword, aesSalt, userId);
//    uint64_t sessionId = generateId();
...
ZErrorCode ZSessionService::create_jwt(const std::string& userId, const std::string& claims, std::string& token) {
    SessionData session_creator(aesPassword, aesSalt, userId);
    uint64_t sessionId = generateId();
    token = session_creator.encode_jwt(sessionId, claims);
    ZDBProxy* dbProxy = ZServiceLocator::instance()->get<ZDBProxy>(ZServiceLocator::ServiceId::DBProxy);
