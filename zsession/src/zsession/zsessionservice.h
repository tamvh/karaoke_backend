...
public:
    ...
    ZErrorCode validate(const std::string& token, int64_t& userId) const;
    ZErrorCode validate_jwt(const std::string& jwt, int64_t& userId) const;

    /// Create new authentication token for given userId
    /// @param token holds authentication token when success
    /// @return ZErrorCode::Authentication_*
    ZErrorCode create(int64_t userId, std::string& token);
    ZErrorCode create_jwt(int64_t userId, const std::string& claims, std::string& token);
    ZErrorCode create_jwt(const std::string& userId, const std::string& claims, std::string& token);
private:
